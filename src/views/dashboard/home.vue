  ...
  <div class="content">
    ...
    <div class="main-content">
      <mt-navbar v-model="selected">
        <mt-tab-item :id="item.id" v-for="item in tabData" :key="item.id">{{item.label}}</mt-tab-item>
      </mt-navbar>
      ...
      <div v-if="list&&list.length">
        <div v-if="selected===firstTabIndex">
        ...
        <div v-if="selected===secondTabIndex">
          <div class="list2" v-for="(item,index) in list" :key="index">
            <div class="first-line">
              <span v-if="selected2!==monDataIndex">{{item.name}}</span>
              <span v-if="selected2===monDataIndex">{{item.monitordate}}</span>
              <span @click="toDetail(item)">详情</span>
            </div>
            <p
              v-if="selected2!==solidWaste&& selected2!==monDataIndex&&item.standard "
            >{{item.standard.name}}</p>
            <p v-if="selected2===solidWaste&& selected2!==monDataIndex">{{item.processingmode}}</p>
            <p v-if="selected2===monDataIndex">{{item.value}}</p>
            <div v-if="selected2!==monDataIndex" class="third-line">
              <span
                v-if="selected2!==noiseIndex&&selected2!==solidWaste"
              >{{item.isonlinemonitoring}}</span>
              <span v-if="selected2===solidWaste">{{item.isdangerousorwaste}}</span>
        ...
        <div v-if="selected===thirdTabIndex">
...
export default {
  ...
  watch: {
    selected() {
      if (this.selected === this.firstTabIndex) {
        //获取污染源信息数据
        this.list = this.pollSourceInfoHeader;
        this.selected2 = "";
        this.getPollSourceList();
      } else if (this.selected === this.secondTabIndex) {
        this.selected2 = `${this.selected}-2`;
        this.list = [];
      } else if (this.selected === this.thirdTabIndex) {
        this.selected2 = `${this.selected}-2`;
        this.list = [];
      }
    },
    selected2() {
      if (this.selected2 === "2-0") {
        ...
        if (this.selected === this.secondTabIndex) {
          this.getOutletList();
        } else if (this.selected === this.thirdTabIndex) {
  ...
  methods: {
    ...
    getMonitorList() {
      ...
      this.$api.getZGDataList(payload).then(res => {
        if (res) {
          res.items.forEach(item => {
            item.monitordate = moment(item.monitordate).format("YYYY-MM-DD");
